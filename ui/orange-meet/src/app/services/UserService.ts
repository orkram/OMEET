...
export class UserService {
  ...
  findContacts(
    ...
    return this.http.get(`http://backend:9000/api/v1/contacts/friends/${username}/page`, {
  ...
  findContactToAdd(filter: string): Observable<any> {
    return this.http.get(`http://backend:9000/api/v1/users/page`, {
      params: new HttpParams()
        .set('query', filter)
  ...
  createConnection(username: string, friendName: string): Observable<any> {
    return this.http.post(`http://backend:9000/api/v1/contacts/add`, '', {
      params: new HttpParams()
